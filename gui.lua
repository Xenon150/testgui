...
function XenonUI:CreateWindow(options)
    ...
    _corner(PKHead,8)
    _new("Frame",{ Size=UDim2.new(1,0,.5,0), Position=UDim2.new(0,0,.5,0), BackgroundColor3=T.BG3, BorderSizePixel=0, ZIndex=5001 }, PKHead)
...
function XenonUI:_makeSection(panel, title, startOpen)
    ...
    local chev = _new("TextLabel",{
        ...
        BackgroundTransparency=1, Text="v", TextColor3=T.Text3,
    ...
    list:GetPropertyChangedSignal("AbsoluteSize"):Connect(function() resize(false) end)
    ...
    function section:AddToggle(a,b,c,d)           return WIN:_makeToggle(list,a,b,c,d) end
    function section:AddSlider(a,b,c,d,e,f)       return WIN:_makeSlider(list,a,b,c,d,e,f) end
    function section:AddButton(a,b)               return WIN:_makeButton(list,a,b) end
    function section:AddDropdown(a,b,c,d,e)       return WIN:_makeDropdown(list,a,b,c,d,e) end
    function section:AddInput(a,b,c)              return WIN:_makeInput(list,a,b,c) end
    function section:AddKeybind(a,b,c,d)          return WIN:_makeKeybind(list,a,b,c,d) end
    function section:AddLabel(text)
        return _new("TextLabel",{
            Size=UDim2.new(1,-8,0,18), BackgroundTransparency=1, Text=text,
            TextColor3=T.Text3, Font=Enum.Font.Gotham, TextSize=10,
            TextXAlignment=Enum.TextXAlignment.Left,
        }, list)
    end
    function section:AddDivider()
        _new("Frame",{ Size=UDim2.new(1,-8,0,1), BackgroundColor3=T.Border, BorderSizePixel=0 }, list)
    end
    return section
end
...
function XenonUI:_makeToggle(parent, label, default, cfgKey, callback)
    ...
    local item = _new("Frame",{
        Size=UDim2.new(1,-8,0,30), BackgroundColor3=T.BG3, BorderSizePixel=0,
    }, parent)
    _corner(item,5)
    ...
    local track = _new("Frame",{
        Size=UDim2.new(0,34,0,17), Position=UDim2.new(1,-44,0.5,-8.5),
        BackgroundColor3=state and T.Accent or T.BG4, BorderSizePixel=0,
    }, item)
    _corner(track,9)
    ...
    local knob = _new("Frame",{
        ...
        BackgroundColor3=state and T.White or T.Text3, BorderSizePixel=0,
    }, track)
    _corner(knob,6)
    ...
    if cfgKey then
        table.insert(WIN._toggleReg,{ cfgKey=cfgKey, ctrl=ctrl, callback=callback })
    end
    ...
    clickBtn.MouseEnter:Connect(function() _tween(item,{BackgroundColor3=T.BG4},.12); _tween(iSt,{Color=T.Border2},.12) end)
    clickBtn.MouseLeave:Connect(function() _tween(item,{BackgroundColor3=T.BG3},.12); _tween(iSt,{Color=T.Border},.12) end)
...
function XenonUI:_makeSlider(parent, label, mn, mx, default, cfgKey, callback)
    ...
    local item = _new("Frame",{
        Size=UDim2.new(1,-8,0,54), BackgroundColor3=T.BG3, BorderSizePixel=0,
    }, parent)
    _corner(item,5)
    local iSt = _stroke(item,T.Border)
    ...
    local fill = _new("Frame",{ Size=UDim2.new(0,0,1,0), BackgroundColor3=T.Accent, BorderSizePixel=0 }, track)
    _corner(fill,9)
    ...
    _new("Frame",{ Size=UDim2.new(0,6,0,6), Position=UDim2.new(.5,-3,.5,-3), BackgroundColor3=T.Accent, BorderSizePixel=0, ZIndex=6 }, knob)
    ...
    if cfgKey then
        table.insert(WIN._sliderReg,{ cfgKey=cfgKey, ctrl=ctrl, callback=callback })
    end

    local dragging = false
    local hitbox = _new("TextButton",{
        Size=UDim2.new(1,0,1,14), Position=UDim2.new(0,0,0,-7),
        BackgroundTransparency=1, Text="", ZIndex=7, AutoButtonColor=false,
    }, track)
    ...
    item.MouseEnter:Connect(function() _tween(item,{BackgroundColor3=T.BG4},.12); _tween(iSt,{Color=T.Border2},.12) end)
    item.MouseLeave:Connect(function() _tween(item,{BackgroundColor3=T.BG3},.12); _tween(iSt,{Color=T.Border},.12) end)
...
function XenonUI:_makeButton(parent, label, callback)
    ...
    local item = _new("Frame",{
        Size=UDim2.new(1,-8,0,30), BackgroundColor3=T.BG3,
        BorderSizePixel=0, ClipsDescendants=true,
    }, parent)
    _corner(item,5)
    local iSt = _stroke(item,T.Border)
    ...
    local hov = _new("Frame",{
        Size=UDim2.new(0,0,1,0), BackgroundColor3=T.AccentDim,
        BackgroundTransparency=.35, BorderSizePixel=0, ZIndex=1,
    }, item)
...
function XenonUI:_makeDropdown(parent, label, opts, default, cfgKey, callback)
    ...
    local ddBox = _new("Frame",{
        ...
        BackgroundColor3=T.BG4, BorderSizePixel=0, ZIndex=11,
    }, item)
    ...
    local arrow = _new("TextLabel",{
        ...
        BackgroundTransparency=1, Text="v", TextColor3=T.Text3,
        Font=Enum.Font.GothamBold, TextSize=10, ZIndex=12, Rotation=0,
    }, ddBox)
    ...
    for _, opt in ipairs(opts) do
        ...
        ob.MouseButton1Click:Connect(function()
            ...
            task.delay(.19,function()
                pcall(function() if list and list.Parent then list.Visible=false end end)
            end)
    ...
    function ctrl:Set(v) sel=v; pcall(function() selTxt.Text=v end) end

    if cfgKey then
        table.insert(WIN._dropdownReg,{ cfgKey=cfgKey, ctrl=ctrl, callback=callback })
    end
...
function XenonUI:_makeInput(parent, placeholder, cfgKey, callback)
    ...
    if cfgKey then
        table.insert(WIN._inputReg,{ cfgKey=cfgKey, box=box, callback=callback })
    end
...
function XenonUI:_makeKeybind(parent, label, bindId, defaultKey, callback)
    ...
    local savedName = self._get("kb_"..bindId, defaultKey and defaultKey.Name or "None")
    ...
    local badge = _new("Frame",{
        ...
        BackgroundColor3=T.BG4, BorderSizePixel=0, ZIndex=3,
    }, item)
